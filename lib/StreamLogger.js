...
class StreamLogger {
    ...
    makeLogPath () {
        if ('string' === typeof this.conf.path) {
            ...
            let logPath = path.join(this.conf.root, template(this.conf.logPathFormat, tplVars));
            if (this.logPathSplit) {
    ...
    createStream (logPath) {
        ...
        let stream = fs.createWriteStream(this.logPath, option)
        stream.on('open', (fd) => {
            this.logFd = fd;
            fs.fstat(this.logFd, (err, stat) => {
                if (err != null) {
                    console.log('lark-log unexpected err : ', err, new Date());
                    process.exit(1);
                    return;
                }
                this.logIno = stat.ino;
                this.watch = setInterval(this.watchLogFile(), 30000);
            });
        });
        stream.on('close', () => {
            this.close();
        ...
        stream.on('error', (err) => {
            console.log(process.pid, ' ', this.logPath, ' bdlog error : ', err, new Date());
            this.close()
            process.exit(1);
        });
        return stream;
    ...
    write (message, method) {
        let context = this.makeContext(message, method);
        let log = template(this.conf.format, context)
