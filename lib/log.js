...
function log(options) {
    var config = options || {};
    var logid = config.logid || Date.now() * 1000 + Math.round(Math.random() * 1000);
    config.format = config.format || "{{logid}} {{timestamp}} <{{title}}> {{file}}:{{line}} ({{method}}) {{message}}";
    config.preprocess = function(data){
        data.logid = logid;
    };
